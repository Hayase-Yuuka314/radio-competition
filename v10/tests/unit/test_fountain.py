...
from wireless_competition.file_protocol.integrity import append_crc32
...
class TestFountainEncoder:
    ...
    def test_systematic_stream(self):
        ...
        for i, pkt in enumerate(encoder.encode_systematic_stream()):
            if pkt.packet_id != HASH_PACKET_ID:
                pkts.append(pkt)
            if len(pkts) >= 10:
                break
        ...
        # payload includes CRC-32 (4 bytes extra)
        assert len(pkts[1].payload) == 128 + 4
