...
| **S2 库模块** | 15 | `channel/*`(4), `adversarial/*`(5), `rx/sim_receiver.py`, `features/time_domain.py`, `ml/*`(2), `fountain/raptorq.py`, `mac/tdd.py`, `contest/dsss_pipeline.py` |
...
### 5.10 `fountain/raptorq.py` — 喷泉码编解码 (LT 码)

**架构**：
```
文件 → [FountainEncoder] → ∞ 编码包流
  ↓ 每个包: (packet_id, file_id, k, total_size, payload)
  k 个系统包 (payload=原始块) → 无限修复包 (payload=XOR of blocks)

接收端:
  收集包 → [FountainDecoder]
  Peel: 找度=1的包 → 恢复块 → 更新其他包 → 重复
  卡住 → Gaussian Elimination (≤20 未知块)
```

**关键参数**：
| 参数 | 默认值 | 含义 |
|---|---|---|
| `block_size` | 256 | 每块字节数, K = ceil(file_size / block_size) |
| `c` | 0.1 | Robust Soliton 鲁棒参数 |
| `delta` | 0.05 | 失败概率界 |
| 开销 | ~5-15% | 需收 K*1.05 ~ K*1.15 包即可解码 |

**有效性验证**：
| 验证项 | 方法 | 状态 |
|---|---|---|
| 零丢包 roundtrip | 系统码 K 包 → decode | ✅ |
| 随机丢包 30% | 发 3xK 包, 随机丢 30% → decode | ✅ |
| 小文件 (2B) | 单块文件, K=1 | ✅ |
| 中文件 (8KB) | K=16, 128B块 | ✅ |
| 确定性 | 相同 file_id → 相同邻居集合 | ✅ |

**现实对接**：block_size 对应比赛文件中约定的分块大小。发射端无限发包直到接收端解码完成（无需 ACK）。

---

### 5.11 `mac/tdd.py` — TDD MAC 时隙调度

**架构**：
```
Superframe (200ms):
  [CCA 5ms] [Guard 5ms] [TX 80ms] [Guard 5ms] [RX 80ms] [Guard 5ms]

CCA: 测信道功率 → < -70dBm → 信道空闲 → 进入 TX
    测信道功率 → >= -70dBm → 信道忙   → backoff, 连续5次黑名单该信道
TX:  发射 SDR 发射, 接收 SDR disable (增益降至 -20dB)
RX:  发射 SDR 静默, 接收 SDR enable
跳频: 16通道 (2.405-2.480 GHz), 每 superframe 跳一次
```
...
|---|---|---|
| CCA 判决 | 信号 vs 噪声功率 | ✅ |
| 跳频序列 | FrequencyHopper 逐跳验证 | ✅ |
| 黑名单 | 超时后自动恢复 | ✅ |
| 时隙统计 | tx/rx/cca 计数 | ✅ |

---

### 5.12 `contest/dsss_pipeline.py` — 比赛 DSSS 管线

**架构**：
```
编码: FountainPacket → ContestDSSSEncoder.encode_packet
  [Guard chips] [Preamble (PN)] [Sync (Gold)] [Payload chips (Gold×bit)]

解码: IQ stream → ContestDSSSDecoder.process_stream
  detect_preamble (滑动相关) → decode_frame (Sync验证 → 解扩 → 包提取)
```

**关键参数**：
| 参数 | 默认 | 含义 |
|---|---|---|
| `spreading_factor` | 128 | Gold 码长度, 21dB 处理增益 |
| `preamble_length` | 256 | 前导码片数 (用于帧检测) |
| `guard_chips` | 64 | 帧间保护间隔 |

**有效性验证**：
| 验证项 | 方法 | 状态 |
|---|---|---|
| Roundtrip | 编码→解码, 所有字段匹配 | ✅ |
| 多包 | 批量编码→解码, ID 匹配 | ✅ |
| 噪声误检 | 纯噪声 → 0 检测 | ✅ |
| 团队隔离 | Team 0 码 ≠ Team 1 码 | ✅ |
| 低 SNR | 噪声 std=0.05×signal → 仍可解码 | ✅ |

**现实对接**：spreading_factor=128 对应 21dB 处理增益，在 -10dB SNR 下可实现 BER < 1e-3。前导长度 256 保证在 16 队同场时可靠帧检测。
