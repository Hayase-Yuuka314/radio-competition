...
class SimulationReceiver:
    ...
    def process_frame(
        ...
        if self.profile.fec_type == FECType.NONE:
            payload_bit_count = min(remaining, (meta.payload_length + crc_overhead) * 8)
        else:
            payload_bit_count = min(remaining // 2, (meta.payload_length + crc_overhead) * 8)

        payload_syms = symbols_full[payload_start:payload_start + remaining]
