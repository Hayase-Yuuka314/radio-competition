...
from ..file_protocol.integrity import append_crc32, check_crc32

HASH_PACKET_ID = 0xFFFFFFFF
_HASH_LEN = 32
...
class FountainEncoder:
    ...
    def __init__(self, data: bytes, block_size: int = 256, seed: int = 0,
        ...
        # CRC-32 on every block for per-packet integrity
        self._blocks: list[bytes] = []
        for i in range(0, len(data), block_size):
            self._blocks.append(append_crc32(data[i:i + block_size]))
        self.k = len(self._blocks)
        self._payload_size = block_size + 4

        # SHA256 of entire file for end-to-end verification
        self._file_hash = hashlib.sha256(data).digest()
    ...
    def next_packet(self) -> FountainPacket:
        ...
        # Special hash packet: send SHA256 once before systematic packets.
        # Re-sent periodically (every 64 packets) to ensure RX gets it.
        if not self._hash_sent or (pid > 0 and pid % 64 == 0):
            self._hash_sent = True
            return FountainPacket(
                packet_id=HASH_PACKET_ID, file_id=self._file_id,
                k=self.k, total_size=self._total_size,
                block_size=_HASH_LEN,
                payload=self._file_hash,
            )
        ...
        return FountainPacket(
            packet_id=pid, file_id=self._file_id, k=self.k,
            total_size=self._total_size, block_size=len(payload),
    ...
    def encode_systematic_stream(self):
        """Generate hash packet + all systematic packets + repair.

        First yields a SHA256 hash packet, then systematic blocks,
        then repair packets.
        """
        # Hash packet first
        yield FountainPacket(
            packet_id=HASH_PACKET_ID, file_id=self._file_id,
            k=self.k, total_size=self._total_size, block_size=_HASH_LEN,
            payload=self._file_hash,
        )

        for i in range(self.k):
            pkt = FountainPacket(
                packet_id=i, file_id=self._file_id, k=self.k,
                total_size=self._total_size, block_size=len(self._blocks[i]),
...
class FountainDecoder:
    ...
    def add_packet(self, packet: FountainPacket):
        ...
        Safety:
        - Hash packet (pid=0xFFFFFFFF): stores SHA256 for final check
        - Systematic packets (pid < k): CRC-32 verified; strips 4B CRC before storing
        - Repair packets (pid >= k): strips 4B CRC residue (CRC not XOR-linear,
          cannot verify; SHA256 at end catches errors)
        """
        pid = packet.packet_id

        if pid == HASH_PACKET_ID:
            if self._expected_hash is None and len(packet.payload) == _HASH_LEN:
                self._expected_hash = bytes(packet.payload)
            return
        ...
        if pid < self._k:
            data, crc_ok = check_crc32(packet.payload)
            if not crc_ok:
                self._crc_bad += 1
                return
        else:
            data = packet.payload[:-4] if len(packet.payload) >= 4 else packet.payload

        self._unique_packet_ids.add(pid)
        self._packets[pid] = data
    ...
    def decode(self) -> Optional[bytes]:
        ...
        Security: verifies SHA256 hash against expected value.
        If hash packet was received and SHA256 doesn't match, returns None.

        Returns:
            Decoded file bytes, or None if decoding fails or integrity check fails.
        """
        if self._k is None:
            return None
        k = self._k
        ...
        result = bytes(result[:total])

        # SHA256 integrity check (if hash packet was received)
        if self._expected_hash is not None:
            computed = hashlib.sha256(result).digest()
            if computed != self._expected_hash:
                return None
