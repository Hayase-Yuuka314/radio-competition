# GNU Radio 竞赛文件链路（独立 PlutoSDR TX/RX）

真实硬件链路已经拆成两份可独立部署的工程：

- [`sdr_contest_tx_pluto.grc`](sdr_contest_tx_pluto.grc)：默认复数正弦占位源或竞赛文件接口，经有限样本与授权门进入 GNU Radio `PlutoSDR Sink`。
- [`sdr_contest_rx_pluto.grc`](sdr_contest_rx_pluto.grc)：GNU Radio `PlutoSDR Source` 经有限采集进入完整同步/FEC/CRC/去重重组接收器，并保存原始 IQ。

两份硬件工程不包含中间仿真信道。原来的 [`sdr_contest_complete.grc`](sdr_contest_complete.grc) 继续保留，作为仿真、IQ 回放和协议回归工程；自定义协议逻辑仍使用 Embedded Python Block，不需要额外 OOT 模块。

## 独立 TX/RX 快速使用

从 RadioConda Prompt 进入本目录：

```powershell
cd C:\Users\16839\Desktop\无线电gpt\gnu_radio_contest_grc
gnuradio-companion sdr_contest_tx_pluto.grc
gnuradio-companion sdr_contest_rx_pluto.grc
```

也可以只编译，不打开 GUI：

```powershell
grcc sdr_contest_tx_pluto.grc sdr_contest_rx_pluto.grc
```

TX 默认 `input_mode = 'placeholder'`，输入为 100 kHz、幅度 0.10 的复基带正弦。开赛拿到文件后，将其改为 `input_mode = 'contest_file'`，再把 `input_path` 指向组委会文件；分块、帧头、CRC、卷积码、BPSK/QPSK 和 RRC 链路会继续使用原实现。

TX 默认保持 `rf_enable = False`、`rules_confirmed = False`、`physical_path_confirmed = False`，`PlutoSDR Sink` 的 `cyclic` 固定为 `False`，并由 `Head` 限制最多发送 `max_tx_samples` 个样本。初始化前置检查会在构造 Pluto Sink 之前验证三重门、`contest_rules.yaml` 及频率/带宽/时长约束，流内授权阀会在调度启动时再次验证；任一条件不满足都会拒绝运行。原生 Pluto 块的 `attenuation1` 使用正衰减值，因此默认 `89.75 dB` 代表最低输出。

RX 默认有限采集 5 秒或 1000 万样本（取较小值），输出：

- `rx_capture.c64`：原始 complex64 IQ；
- `decoded_payload.bin`：全部块齐全且 CRC 通过后的重组文件；
- `run_metrics.json`：检测、CRC、重复帧和缺块分类指标；
- `decoded_payload.bin.parts/`：接收不完整时保存的已验证分块。

收发两端的中心频率、采样率、调制、FEC、每符号采样数、RRC、前导、保护间隔和 `seed` 必须一致。`device_uri` 必须填写 `iio_info -S usb` 或网络发现实际返回的 URI，不能依赖“第一台设备”的枚举顺序。
