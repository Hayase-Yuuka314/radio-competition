...
class ContestDSSSDecoder:
    ...
    def decode_frame(self, chips: np.ndarray, preamble_pos: int,
        ...
        sync_corr = np.abs(np.dot(sync_seg, self._sync_code))
        if sync_corr < 0.5 * sync_len:
            return None

        pos += sync_len

        # Phase 1: read just the header bits (PACKET_HEADER_SIZE bytes × 8)
        ...
        for _ in range(header_bits_needed):
            ...
            soft = np.dot(chip_seg, self._data_code)
    ...
    def process_stream(self, chips: np.ndarray) -> list[FountainPacket]:
        ...
        Two-pass approach:
        1. Find preamble candidates via sliding correlation (low threshold)
        2. Validate each candidate by attempting frame decode (sync word + header)
        3. Return all successfully decoded packets
        """
        ...
        for idx in candidate_indices:
            ...
            pkt = self.decode_frame(chip_arr, idx)
            if pkt is not None:
                packets.append(pkt)
