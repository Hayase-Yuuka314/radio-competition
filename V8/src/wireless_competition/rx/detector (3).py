...
class FrameDetector:
    ...
    def detect_sample_rate(
        ...
        与 :meth:`detect` 的符号率索引不同，本方法返回原始 ``iq_signal``
        的样本索引，适合连续流切帧。绝对阈值抑制噪声，相对最大峰阈值
        抑制 payload 内偶然出现的次相关峰。
        """
        ...
        effective = max(float(self.threshold), float(relative_threshold) * peak_max)
        window = len(self.preamble_shaped)
        if min_distance_samples <= 0:
            min_distance_samples = window
        ...
        while i < len(corr):
            ...
            j = i
            peak = i
            while j < len(corr) and corr[j] >= effective:
                if corr[j] > corr[peak]:
                    peak = j
                j += 1
            if not peak_indices or peak - peak_indices[-1] >= min_distance_samples:
                peak_indices.append(peak)
            i = max(j, peak + min_distance_samples // 2)

        return [max(0, peak - window + 1) for peak in peak_indices]
