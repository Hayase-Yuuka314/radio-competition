...
> **项目定位**：基于 SDR 的混合式智能通信系统 — 喷泉码无ACK传输 + DSSS 扩频抗干扰 + TDD MAC 时分调度。
> **测试**：96/99 全部通过 (3 预存 GRC 测试失败与本项目无关) | **语言**：Python 3.11+ | **核心依赖**：NumPy/SciPy/scikit-learn
...
```
比赛模式 (无ACK, 喷泉码):
  文件 → [Fountain Encoder] → ∞ 编码包
           ↓
  [Contest DSSS: Gold码 SF=128, 21dB PG] → IQ 波形
           ↓
  [TDD MAC: CCA → TX → Guard → RX, 16ch 跳频]
           ↓
  PlutoSDR TX (+2.4GHz BPF) ～～～ 空中 ～～～ PlutoSDR RX (裸)
           ↓
  [DSSS Decoder: 前导搜索 → 解扩 → 喷泉包提取]
           ↓
  [Fountain Decoder: 收够 K+overhead 包 → 解码 → 文件]

仿真模式 (传统 BPSK/QPSK):
  文件 → [TX Pipeline] → [虚拟信道: 噪声+干扰+损伤] → [RX Pipeline] → 文件
```

**一句话**：比赛模式用喷泉码 + DSSS + TDD 实现无 ACK 可靠传输。仿真模式保留了完整的传统 BPSK/QPSK/OFDM 链路。
...
# 比赛模式端到端仿真 (喷泉码 + DSSS)
python -m wireless_competition.cli.transmit test_data.bin --team-id 0 --sim --output recovered.bin
...
```bash
# 发射端 (连接 PlutoSDR TX + 2.4GHz 滤波器)
wireless-transmit <input_file> --team-id 0 --freq 2450e6

# 接收端 (连接 PlutoSDR RX, 无外置滤波器)
wireless-receive --team-id 0 --freq 2450e6 --output recovered.bin

# 仿真模式 (无需硬件, TX→信道→RX 全内存)
wireless-transmit <input_file> --team-id 0 --sim --sim-snr 15 --output recovered.bin
```
...
📁 项目根目录
├── 📄 README.md          ← 你正在看
├── 📄 ARCHITECTURE.md    ← 软件架构全景图
├── 📄 SIMULATION.md      ← 模拟程序架构+有效性论证+域迁移
├── 📄 STATUS.md          ← 当前进度+测试统计
├── 📄 DECISIONS.md       ← 架构决策记录(ADR, 含喷泉码+Gold码统一)
│
├── 📁 configs/           ← 配置文件（调参改这里）
...
│   ├── adversarial/      ← ⚔️ DSSS对抗：扩频/标准Gold码/策略/评估
│   ├── fountain/         ← 💧 喷泉码：LT码 (Robust Soliton), 无ACK传输
│   ├── mac/              ← ⏱ TDD MAC：CCA/跳频/时隙调度/黑名单
│   ├── contest/          ← 🏆 比赛管线：DSSS编解码器/编排器
│   ├── ml/               ← 🤖 机器学习：数据集/随机森林
│   ├── features/         ← 📐 特征提取：时域+频域+接收机
│   ├── evaluation/       ← 📊 评估：指标/MonteCarlo/多队模拟
│   ├── policy/           ← 🎯 策略：自适应联动
│   ├── sdr/              ← 📡 SDR：抽象接口+仿真器+PlutoSDR驱动
