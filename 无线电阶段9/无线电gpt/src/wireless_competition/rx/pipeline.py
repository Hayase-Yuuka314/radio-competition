...
class Receiver:
    ...
    def process(
        ...
        # 在原始采样率上定位前导，避免把符号索引误当作采样索引。
        preamble_starts = self._detector.detect_sample_rate(iq)
        if not preamble_starts:
            return [DecodeResult(
        ...
        # 协议 v1 的默认保护间隔为 16 符号。相关定位的是前导首样本，
        # 因此向前包含 guard；相邻前导给出上一帧的自然结束边界。
        guard_samples = 16 * sps
        frame_starts = [max(0, start - guard_samples) for start in preamble_starts]

        from .sim_receiver import SimulationReceiver

        aligned_receiver = SimulationReceiver(profile=self.profile)
        ...
        for index, frame_start in enumerate(frame_starts):
            ...
            if frame_end <= frame_start:
                continue
            results.append(aligned_receiver.process_frame(
                iq[frame_start:frame_end], sample_rate_hz, guard_symbols=16
            ))
    ...
    def _process_single_frame(
        ...
        raw_payload_bits = (meta.payload_length + 4) * 8
        ...
        payload_syms = symbols[payload_start:payload_start + payload_symbol_count]
