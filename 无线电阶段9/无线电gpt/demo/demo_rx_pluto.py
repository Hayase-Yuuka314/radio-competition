...
# Title: Demo RX - DSSS protected text over PlutoSDR
# Author: OpenAI Codex / wireless demo
# Copyright: 2026
# Description: PlutoSDR -> 突发同步/CFO -> DSSS 解扩 -> 去交织/维特比 -> 解扰/CRC
...
import demo_rx_pluto_demo_rx_sink as demo_rx_sink  # embedded python block
import sip
import threading



class demo_rx_pluto(gr.top_block, Qt.QWidget):

    def __init__(self):
        gr.top_block.__init__(self, "Demo RX - DSSS protected text over PlutoSDR", catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle("Demo RX - DSSS protected text over PlutoSDR")
        ...
        self.settings = Qt.QSettings("gnuradio/flowgraphs", "demo_rx_pluto")
        ...
        self.output_text_path = output_text_path = 'decoded_message.txt'
        self.output_json_path = output_json_path = 'rx_status.json'
        self.expected_sequence = expected_sequence = 1
        self.device_uri = device_uri = 'usb:1.3.5'
        self.code_length = code_length = 31
        self.center_frequency_hz = center_frequency_hz = 433000000
        ...
        self.demo_rx_sink = demo_rx_sink.blk(shared_key=shared_key, sample_rate=samp_rate, samples_per_chip=samples_per_chip, code_length=code_length, team_id=team_id, expected_sequence=expected_sequence, output_text_path=r'C:/Users/16839/Desktop/rxoutput.txt', output_json_path=r'C:/Users/16839/Desktop/rx_status.json')


        ##################################################
        # Connections
        ##################################################
        self.connect((self.finite_capture, 0), (self.demo_rx_sink, 0))
        self.connect((self.finite_capture, 0), (self.qtgui_freq_sink_x_0, 0))
        self.connect((self.finite_capture, 0), (self.qtgui_time_sink_x_0, 0))
    ...
    def set_output_json_path(self, output_json_path):
        self.output_json_path = output_json_path

    def get_expected_sequence(self):
        return self.expected_sequence

    def set_expected_sequence(self, expected_sequence):
...
def main(top_block_cls=demo_rx_pluto, options=None):
