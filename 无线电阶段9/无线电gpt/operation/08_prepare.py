...
try:
    ...
    # pyadi-iio may be installed while its native libiio DLL is unavailable.
    # Treat that as an unusable optional hardware backend instead of aborting
    # the entire offline preparation report.
    print(f"  [INFO] pyadi-iio 当前不可用 ({type(exc).__name__}: {exc})")
